...
import { ChevronLeft, ChevronRight, Calendar, ArrowLeft, Sparkles, Search, Eye, User, Clock, TrendingUp, ExternalLink, BarChart3, ArrowRight } from 'lucide-react';
...
const CounterAnimation = ({ target, suffix = "", duration = 2000, delay = 0 }: CounterAnimationProps) => {
  ...
  return (
    <motion.div
      ...
      className="text-4xl md:text-5xl font-bold bg-gradient-to-r from-amber-400 via-yellow-500 to-amber-600 bg-clip-text text-transparent mb-2"
    >
      <motion.span
        animate={{
...
const KristalinNewsPage: React.FC = () => {
  ...
  const newsPerPage = 6;
  const categories = ['All', 'Mining Operations', 'CSR Initiative', 'Environmental', 'Achievement', 'Community Development', 'Partnership', 'Technology'];
          ...
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
            ...
            <div className="lg:col-span-2">
              ...
              {selectedNews.metrics && (
                <div className="bg-gray-900/50 backdrop-blur-sm border border-amber-500/20 rounded-2xl p-4 mt-4 w-full">
              ...
              <div className="mt-8 pt-6 border-t border-gray-800">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {newsData.slice(0, 2).map((relatedNews) => (
                    <div key={relatedNews.id} className="group bg-gradient-to-r from-yellow-400/20 to-yellow-500/20 border border-yellow-400/30 rounded-lg p-3 cursor-pointer hover:from-yellow-400/30 hover:to-yellow-500/30 transition-all duration-300">
                      <div className="text-xs mb-1 text-yellow-400 font-medium">{relatedNews.category} • {relatedNews.date}</div>
                      <div className="font-semibold text-white text-xs mb-2 group-hover:text-yellow-400 transition-colors line-clamp-2">
                        {relatedNews.title}
                      </div>
                      <div className="flex items-center justify-between">
                        <span className="text-gray-400 text-xs">{relatedNews.readTime}</span>
  ...
  return (
    <div className="min-h-screen bg-white">
          ...
          >
            ...
            >
              <div className="relative group">
                <Search className="absolute left-4 top-1/2 transform -translate-y-1/2 text-amber-400 w-5 h-5 z-10" />
                <input
                  type="text"
                  placeholder="Search mining news..."
                  value={searchQuery}
                  onChange={(e) => setSearchQuery(e.target.value)}
                  className="w-full pl-12 pr-4 py-4 bg-black/30 backdrop-blur-md border border-amber-400/30 rounded-xl text-white placeholder-gray-300 focus:ring-2 focus:ring-amber-400 focus:border-transparent transition-all duration-300"
                />
              </div>
            </motion.div>
                ...
                <motion.button
                  ...
                  className={`px-6 py-3 rounded-full text-sm font-medium transition-all duration-300 ${
                    selectedCategory === category
                      ? 'bg-gradient-to-r from-amber-400 to-yellow-500 text-black font-bold shadow-lg'
                      : 'bg-white/10 backdrop-blur-sm border border-white/20 text-white hover:text-amber-400 hover:bg-white/20'
                  }`}
                >
                  {category}
                </motion.button>
      ...
      <motion.section
        ...
        className="py-12 bg-gradient-to-br from-gray-50 via-white to-amber-50/30 relative overflow-hidden"
      >
        {/* Background Pattern */}
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-8 mb-8">
            {[
              ...
              <motion.div
                ...
                className="group text-center p-8 bg-white/90 backdrop-blur-sm border border-gray-200 rounded-2xl transition-all duration-500 relative overflow-hidden hover:bg-white hover:shadow-lg"
              >
                ...
                <div className="relative z-10">
                  ...
                  <h4 className="text-lg font-semibold text-gray-800 group-hover:text-amber-600 transition-colors duration-300 mb-3">
                    {stat.label}
                  </h4>
      ...
      <motion.section
        ...
        whileInView="visible"
        viewport={{ once: true, amount: 0.2 }}
        className="py-12 bg-gradient-to-br from-white via-gray-50 to-white"
      >
          ...
          >
            <div className="flex items-center space-x-6">
              <motion.button
                ...
                whileTap={{ scale: 0.95 }}
                className="group w-14 h-14 rounded-full bg-white border border-amber-200 hover:bg-amber-50 disabled:opacity-30 disabled:cursor-not-allowed flex items-center justify-center transition-all duration-300 shadow-sm hover:shadow-md"
              >
                <ChevronLeft className="w-6 h-6 text-amber-600 group-hover:-translate-x-0.5 transition-transform" />
              </motion.button>

              <div className="text-gray-800 text-2xl font-light flex items-center space-x-3">
                <span className="text-amber-600 font-semibold">{currentPage}</span>
              ...
              <motion.button
                ...
                className="group w-14 h-14 rounded-full bg-white border border-amber-200 hover:bg-amber-50 disabled:opacity-30 disabled:cursor-not-allowed flex items-center justify-center transition-all duration-300 shadow-sm hover:shadow-md"
              >
                <ChevronRight className="w-6 h-6 text-amber-600 group-hover:translate-x-0.5 transition-transform" />
              </motion.button>
            </div>
          </motion.div>
          ...
          >
            {currentNews.map(news => (
              <motion.div
                ...
                className="bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-500 flex flex-col border border-gray-100 h-full"
              >
                {/* Gambar dengan badge kategori */}
                  ...
                  <div className="absolute top-4 left-4">
                    <span
                      className={`bg-gradient-to-r ${getCategoryColor(news.category)} text-white px-3 py-1.5 rounded-full text-xs font-bold shadow-lg`}
                    >
                        {news.category}
                      </span>
                        </div>
                ...
                {/* Konten */}
                <div className="group relative p-4 hover:before:w-full before:absolute before:bottom-0 before:left-0 before:h-[2px] before:w-0 before:bg-yellow-400 before:transition-all before:duration-300 flex flex-col flex-1 leading-snug">
                  ...
                  {/* Judul & Subjudul */}
                  <h3 className="text-lg font-bold text-gray-900 mb-1 group-hover:text-amber-700 transition-colors duration-300 line-clamp-2">
                      {news.title}
                    </h3>
                    {news.subtitle && (
                    <h4 className="text-sm text-amber-600 mb-2 line-clamp-1">
                        {news.subtitle}
                      </h4>
                    )}
                  {/* Excerpt */}
                  ...
                  {/* Progress bar */}
                      <div className="w-full bg-gray-200 rounded-full h-1">
                        <div className="bg-gradient-to-r from-amber-400 to-yellow-500 h-1 rounded-full w-0 group-hover:w-full transition-all duration-1000 ease-out"></div>
                  </div>
                </div>
              </motion.div>
            ))}
